  ...
  return (
    <motion.div
      ...
      style={{
        ...
        offsetRotate: '0deg', // Keeps children upright!
...
export default function OrbitImages({
  ...
  const items = images.map((src, index) => (
    <div key={src} className="w-full h-full rounded-2xl overflow-hidden border-4 border-white shadow-xl shadow-black/10 bg-white group">
        <img
            src={src}
            alt={`${altPrefix} ${index + 1}`}
            draggable={false}
            className="orbit-image group-hover:scale-110 transition-transform duration-700"
